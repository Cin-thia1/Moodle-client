...
<div class="bg-gray-50 min-h-screen">
    <div class="container mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div class="max-w-2xl bg-white rounded-lg shadow-lg p-8">
            <h1 class="text-3xl font-bold text-gray-900 mb-6">Modifier une compétence</h1>

            <form action="{{ route('competencies.update', $competency) }}" method="POST" class="space-y-6">
                @csrf @method('PATCH')
                ...
                <div>
                    ...
                    <input type="text" id="shortname" name="shortname" required
                        ...
                        value="{{ old('shortname', $competency->shortname) }}">
                    @error('shortname')
                ...
                <div>
                    ...
                    <input type="text" id="idnumber" name="idnumber" required
                        ...
                        value="{{ old('idnumber', $competency->idnumber) }}">
                    @error('idnumber')
                    <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="5"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 @error('description') border-red-500 @enderror">{{ old('description', $competency->description) }}</textarea>
                    @error('description')
                    <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <select id="status" name="status"
                        ...
                        <option value="1" {{ $competency->status ? 'selected' : '' }}>Actif</option>
                        <option value="0" {{ !$competency->status ? 'selected' : '' }}>Inactif</option>
                    </select>
                </div>
