...
<div class="max-w-3xl mx-auto px-4 py-6">
  ...
  {{-- Header avec timer si durée limitée --}}
  <div class="bg-white rounded-lg shadow p-4 mb-6 flex items-center justify-between">
    <div>
      ...
      <p class="text-xs text-gray-500">Tentative {{ $attempt->attempt }}</p>
    </div>
    @if($module->timelimit)
      <div class="text-right">
        <div class="text-xs text-gray-500 mb-1">Temps restant</div>
        <div id="timer" class="text-xl font-mono font-bold text-red-600"></div>
      </div>
    @endif
  </div>
  ...
  <form action="{{ route('quiz.submit', $module->id) }}" method="POST" id="quiz-attempt-form">
        ...
        <div class="bg-white rounded-lg shadow p-5">
          <div class="flex items-start justify-between gap-3 mb-4">
            <div class="flex items-center gap-2">
              <span class="text-xs font-semibold
                {{ $question->qtype === 'truefalse' ? 'bg-indigo-100 text-indigo-700' : 'bg-blue-100 text-blue-700' }}
                px-2 py-0.5 rounded">
          ...
          <div class="space-y-2">
            @foreach($question->answers as $answer)
              ...
              @endphp
              <label class="flex items-center gap-3 px-3 py-2.5 rounded-md border cursor-pointer
                            hover:bg-blue-50 hover:border-blue-300 transition
                            {{ $isGiven ? 'bg-blue-50 border-blue-400' : 'border-gray-200' }}">
                <input type="radio"
    ...
    <div class="mt-6 flex justify-between items-center">
      <a href="{{ route('quiz.show', $module->id) }}"
         ...
         onclick="return confirm('Quitter sans soumettre ? Votre progression sera perdue.')">
        Quitter
      </a>
      <button type="submit"
