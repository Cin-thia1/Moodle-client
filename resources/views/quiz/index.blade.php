        ...
        <aside class="col-span-12 md:col-span-3">
            ...
            @if($isTeacher)
                <div class="mt-4 space-y-2">
                    ...
                    {{-- ✅ route correcte : quiz.create (pas quizzes.create) --}}
                                ...
                                <div class="flex items-start justify-between gap-4">
                                    ...
                                    <div class="min-w-0 flex-1">
                                        <div class="flex items-center gap-2">
                                            @if($isQuiz)
                                                <span class="text-xs bg-indigo-100 text-indigo-700 px-2 py-0.5 rounded font-medium shrink-0">QUIZ</span>
                                            @else
                                                <span class="text-xs bg-blue-100 text-blue-700 px-2 py-0.5 rounded font-medium shrink-0">DEVOIR</span>
                                            @endif

                                            {{-- ✅ routes corrigées : quiz.show et assignments.show --}}
                                        ...
                                        <div class="text-xs text-gray-500 mt-1">
                                            @if($isQuiz)
                                                @if($a->timeopen)
                                                    Ouvert : <span class="font-medium text-gray-700">{{ \Carbon\Carbon::parse($a->timeopen)->setTimezone(config('app.timezone'))->format('d/m/Y H:i') }}</span>
                                                @endif
                                                @if($a->timeclose)
                                                    • Fermé le : <span class="font-medium text-gray-700">{{ \Carbon\Carbon::parse($a->timeclose)->setTimezone(config('app.timezone'))->format('d/m/Y H:i') }}</span>
                                                @endif
                                                @if($a->timelimit)
                                                    • ⏱ <span class="font-medium text-gray-700">{{ floor($a->timelimit / 60) }} min</span>
                                                @endif
                                                • Note max : <span class="font-medium text-gray-700">{{ $a->grade ?? 20 }}</span>
                                            @else
                                                Date limite :
                                        ...
                                        {{-- Statut élève pour les devoirs --}}
                                        @if(!$isTeacher && !$isQuiz)
                                            @php $sub = $mySubs[$a->id] ?? null; @endphp
                                            <div class="flex flex-wrap gap-2 mt-2">
                                                @if(!$sub)
                                                    <span class="text-xs bg-yellow-100 text-yellow-700 px-2 py-1 rounded">Pas encore remis</span>
